...
describe("useUndoRedoFeature", () => {
  ...
  it("should push action to undo stack", () => {
    ...
    const action = createAction("UPDATE", mockRow1, mockRow1);
  ...
  it("should push multiple actions to undo stack", () => {
    ...
    const action1 = createAction("ADD", undefined, mockRow1);
    const action2 = createAction("UPDATE", mockRow2, mockRow2);
  ...
  it("should limit undo stack to maxSteps", () => {
    ...
    act(() => {
      for (let i = 0; i < 5; i++) {
        const row = { id: `${i}`, name: `Row ${i}`, value: i };
        result.current.actions.pushAction(createAction("UPDATE", row, row));
      }
    });

    expect(result.current.state.undoStack).toHaveLength(3);
    // Should keep the last 3 actions
    expect(result.current.state.undoStack[0]?.after?.id).toBe("2");
    expect(result.current.state.undoStack[1]?.after?.id).toBe("3");
    expect(result.current.state.undoStack[2]?.after?.id).toBe("4");
  });
  ...
  it("should only track specified action types", () => {
    ...
    const updateAction = createAction("UPDATE", mockRow1, mockRow1);
    const addAction = createAction("ADD", undefined, mockRow2);
  ...
  it("should undo last action", () => {
    ...
    expect(redoAction?.before).toEqual(action.before);
    expect(redoAction?.after).toEqual(action.after);
  });
  ...
  it("should redo last undone action", () => {
    ...
    expect(undoAction?.before).toEqual(action.before);
    expect(undoAction?.after).toEqual(action.after);
  });
  ...
  it("should clear redo stack when new action is pushed", () => {
    ...
    const action1 = createAction("UPDATE", mockRow1, mockRow1);
    const action2 = createAction("UPDATE", mockRow2, mockRow2);
    ...
    expect(undoAction?.before).toEqual(action2.before);
    expect(undoAction?.after).toEqual(action2.after);
  });
  ...
  it("should clear all history", () => {
    ...
    act(() => {
      result.current.actions.pushAction(
        createAction("UPDATE", mockRow1, mockRow1),
      );
      result.current.actions.pushAction(
        createAction("UPDATE", mockRow2, mockRow2),
      );
    });
