import { pgTable, unique, uuid, varchar, timestamp, foreignKey, index, text, boolean, integer, primaryKey } from "drizzle-orm/pg-core"
...
export const workLogs = pgTable("work_logs", {
	...
	index("work_logs_category_id_idx").using("btree", table.categoryId.asc().nullsLast().op("uuid_ops")),
	index("work_logs_project_id_idx").using("btree", table.projectId.asc().nullsLast().op("uuid_ops")),
	index("work_logs_user_id_date_idx").using("btree", table.userId.asc().nullsLast().op("uuid_ops"), table.date.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const projects = pgTable("projects", {
	...
	index("projects_is_active_idx").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
...
export const workCategories = pgTable("work_categories", {
	...
	index("work_categories_is_active_display_order_idx").using("btree", table.isActive.asc().nullsLast().op("int4_ops"), table.displayOrder.asc().nullsLast().op("int4_ops")),
